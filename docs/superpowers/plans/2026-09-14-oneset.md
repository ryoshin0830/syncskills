# oneset Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship `npx oneset`, a CLI that keeps AI-agent skills and MCP servers identical across several machines by three-way syncing cc-switch's data through a GitHub repository, with secrets held in 1Password.
...
**Spec:** `docs/superpowers/specs/2026-09-14-oneset-design.md`

## Global Constraints

- Package name `oneset`; bins `oneset` and `ssync`. Repo `github.com/ryoshin0830/syncskills`.
...
- Config directory is `~/.config/oneset` (override: `--config`, or `$ONESET_CONFIG_DIR`).
...
```json
{
  ...
  "bin": { "oneset": "./dist/cli.js", "ssync": "./dist/cli.js" },
...
const sample = {
  schemaVersion: 1 as const, host: 'github.com', owner: 'ryoshin0830', repo: 'oneset',
...
export async function loadConfig(dir: string): Promise<Config | null> {
  ...
  if (parsed.schemaVersion !== 1) {
    throw new Error(
      `unsupported config schema version ${parsed.schemaVersion}; upgrade oneset`,
    )
  }
  return parsed
}
...
credential helper; no token is ever read by oneset.
...
EXAMPLES
  oneset init
  oneset init --host github.com --repo me/oneset --vault agent --device work-pc --json`,
...
EXAMPLES
  ...
  oneset sync --only mcp --merge-agent codex`,
...
export async function runInit(opts: {
  ...
  p.intro('oneset — set up this device')
  ...
  const [owner = chosen.login, repo = 'oneset'] = repoAnswer.split('/')
  ...
  if (useSecrets) {
    ...
    const probe = onePasswordProvider({ vault, item: 'oneset', token })
  ...
  const config: Config = {
    ...
    device, vault, item: 'oneset', secrets: useSecrets, excludes: [],
  }
  ...
  p.outro('Ready. Run `oneset` to sync.')
  return config
}
```
...
describe('built CLI', () => {
  ...
  it('exits 3 when not initialized', async () => {
    const r = await run('node', ['dist/cli.js', 'status', '--json'], {
      env: { ONESET_CONFIG_DIR: '/tmp/oneset-nonexistent-' + Date.now() },
    })
...
(`npx oneset init`, then `npx oneset`); how the three-way sync behaves,
...
how to recover from a bad merge with `oneset conflicts`.
